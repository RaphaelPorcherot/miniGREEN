...
SHIFT_nominalExport <- function(){
    eq({# g exp i[ind]= exogenous exp growth. Prev : 0.6
        ...
        }else{# Previous magic number : exp_initial_i[ind]* (1 - elast_p_exp * inflation_i[ind]) +(g_exp_i[ind]*Time*convergence)
...
POL_importShareInIntermediateTrade <- function(){
    eq({#share imp Z ii[ind,toind] share of imports with respect to domestic in intermediate trade
        start <- gp("startPolicy")
        end <- gp("endPolicy")
...
SHIFT_nominalImportIntermediateDemand <- function(){
    eq({#Z imp nom i[toind]=sum(p i[ind!]*Z imp real ii[ind!,toind])*IF THEN ELSE(Time=Time 1st import price shock,1+import price shock*Act Price Shock,1)*IF THEN ELSE(Time=Time shock,1+import price shock*Act Price Shock,1)
        start <- gp("startPolicy")
        end <- gp("endPolicy")
...
realExport <- function(){
    eq({#total exp real i[ind]= total exp nom i[ind]/p i[ind]
...
realImportInterMediateDemand <- function(){
    eq({#Z imp real ii[ind,toind]= share imp Z ii[ind,toind] * Z real ii[ind,toind]
...
totalRealImportIntermediateDemand <- function(){
    eq({#Z imp real tot=sum(Z imp real ii[ind!,ind!])

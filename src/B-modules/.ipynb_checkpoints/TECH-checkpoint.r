...
computeAlternativeChangeLabourProductivity <- function(){
    eq({
        ...
        R_gLabProdAlt_iv[,2] <- R_scalingLabProd * rtruncnorm(n = 1, a = min + sd, b = max + sd, mean = max(0, mean) + sd, sd = sd)
        # delta lambda T3 i : change in labour productivity for T3
        R_gLabProdAlt_iv[,3] <- R_scalingLabProd * rtruncnorm(1, min - sd, max - sd, max(0, mean) - sd, sd)
        # delta lambda T4 i : change in labour productivity for T4
        R_gLabProdAlt_iv[,4] <- R_scalingLabProd * rtruncnorm(1, min + sd, max + sd,max(0, mean) + sd, sd)
        R_gLabProdAlt_iv
...
computeAlternativeDesiredLabour <- function() {
    eq({# L T iv - unlike L i, it is not used elsewhere : there is no need to make a separate function for it. 
        ...
        for (v in technology) {
            ...
            for(g in gender){
                ST_desLabAlt_isvg[,,v,"male"] <- ST_desLabAlt_isv[,,v] * SH_male_is_lvl
                SH_female_is_lag <- 1 - SH_male_is_lvl
                ST_desLabAlt_isvg[,,v,"female"] <- ST_desLabAlt_isv[,,v] * SH_female_is_lag
            }
        }
        ST_desLabAlt_isvg
    })
...
computeAlternativeGrossWageBill <- function(){
    eq({#GWB iv
        ...
        for(v in technology){
            F_GWBAlt_isvg[,,v,] <- R_hrWage_isg_lvl * F_labHr_i * ST_desLabAlt_isvg[,,v,]
        }
...
techFrontierLabourProd <- function() {
    eq({# techn frontier lambda i
        R_techFrontLadProd_i <- R_labProd_i_lag + (R_labProd_i_lag - R_labProd_i_lag2)
        R_techFrontLadProd_i
    })
}

currTechDiffusionLabourProd <- function() {
    eq({# lambda diffusion i

...
├── DIFF-2025-2026.md  what the 2026 model changed (generated)
├── inconsistencies_new.md  problems found while refactoring
...
Two scripts answer questions that are easy to get wrong by hand and expensive to
get wrong at all.

```bash
Rscript tool/check-mapping.r            # is MAPPING.md complete, both ways?
Rscript tool/check-mapping.r --update   # refresh its counters and derived list
Rscript tool/diff-models.r              # regenerate DIFF-2025-2026.md
```

`check-mapping.r` asks four questions: does every R function have a row, does
every row name a function that exists, is every Vensim 2026 variable either
claimed by a row or listed as untranslated, and does every name a row claims
exist in the 2026 model. It never touches the confidence of a row — that is a
human judgement. `--update` refreshes only the counters and the list of
unclaimed variables, which are derived and go stale on their own.

`diff-models.r` compares the two Vensim models equation by equation. The number
to look at is not how many variables were removed but **how many kept their name
and had their equation rewritten** — 231, of which 55 already have an R
translation. Those 55 look current and are not, and nothing in the code will say
so. `DIFF-2025-2026.md` opens with them.
...
* ~~A 2025 ↔ 2026 diff.~~ `DIFF-2025-2026.md`, generated by
  `Rscript tool/diff-models.r`.
